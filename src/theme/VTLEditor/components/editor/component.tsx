...
const Editor = ({
	...
	useEffect(() => {
		if (movedCursor) {
			...
			monacoRef?.current?.editor?.setPosition(
				new Position(movedCursor.line, movedCursor.column)
			);
		}
	...
	const parseContent = useCallback(
		(t: CustomTools) => {
			...
			if (editor) {
				const monacoErrors: EditorApi.editor.IMarkerData[] = validate(t)(
					editor.getValue()
				).map((error: any) => {
					return {
				...
				if (model)
					EditorApi.editor.setModelMarkers(model, 'owner', monacoErrors);
				if (onListErrors) {
					onListErrors(
	...
	useEffect(() => {
		if (
			(!Array.isArray(variableURLs) || variableURLs.length === 0) &&
			!sdmxResultURL
		)
			setReady(true);
	...
	return (
		<div className="editor-container" style={buildStyle(options)}>
			<MonacoEditor
				ref={monacoRef}
				editorWillMount={getEditorWillMount(tools)({
					variables: vars,
					sdmxResult: sdmxRes,
				})}
